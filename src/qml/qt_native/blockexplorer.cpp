...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::GKC, nValue);
...
#ifdef DPOS
        return Params().GenesisBlock().GetHash().GetHex();
#else
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if(crp::IsPowHeight(pBlock->nHeight))
    {
...
//            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (gkc.conf).");
//            QMessageBox::warning(this, "GKC Core Blockchain Explorer", Warning, QMessageBox::Ok);
//        }
//    }
}
