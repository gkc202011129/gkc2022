...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
				...
				if (EnableBanAddress()) {
					fHasBanInput = HasAddressBanned(coins->vout[txin.prevout.n].GetScriptAddresses(), chainActive.Height());
					if (fHasBanInput) {
						break;
					}
				}
            ...
            if (fMissingInputs || fHasBanInput) continue;
