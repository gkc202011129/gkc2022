...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "gkc.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "gkcd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GKC and zGKC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-maxconnections=<n>", strprintf(_("Maintain at most <n> connections to peers (default: %u)"), 4096));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GKC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GKC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, gkc, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GKC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GKC specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
//    strUsage += "  -anonymizegkcamount=<n>     " + strprintf(_("Keep N GKC anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
            ...
            "\n" +
            FormatParagraph(strprintf(_("Copyright (C) 2017-%i The GKC Core Developers"), COPYRIGHT_YEAR)) + "\n" +
            "\n" +
            ...
            "\n" + 
			FormatParagraph(_("Block browser <explorer.gkc.cash>.")) + "\n" +
			"\n" + 
			FormatParagraph(_("GKC download address <download.gkc.cash>.")) + "\n" ;
}
...
/** Sanity checks
 *  Ensure that GKC is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (GetBoolArg("-banappnode", false)) {
        BanAppNode("bread");
    }

    if (mapMultiArgs.count("-banaddress") > 0) {
        InitBanAddressMap(mapMultiArgs["-banaddress"]);
    }

#ifdef DPOS
    if (mapArgs.count("-agentid")) {
		//委托节点ID
		AgentID agentid;
		agentid.SetHex(mapArgs["-agentid"]);
    ...
    nMaxConnections = GetArg("-maxconnections", 4096);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GKC Core is shutting down."));
...
#endif
    // Make sure only a single GKC process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GKC Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        nWalletBackups = GetArg("-createwalletbackups", 100000000);
        if (nWalletBackups > 0) {
            if (filesystem::exists(backupDir)) {
    ...
    // *********************************************************
    if(!dpos::LoadEntrustmentCache())
		return InitError("dpos::LoadEntrustmentCache() error");
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex){
                    //TODO:gkc-evm ,fReindex is true ,wipeResults
                ...
                // GKC: load previous sessions sporks if we have them.
                ...
                //NOTE: Start CContractComponent at a proper moment,
                //this may not be the best time, needs check,
                //checked with gkc codes. gkc-vm
                ContractInit();
...
//                }
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZGKCMinted();
                        RecalculateZGKCSpent();
                    }
                    RecalculateGKCSupply(1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GKC Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart GKC Core to complete") << "\n";
...
//    }
//
//    nAnonymizeGkcAmount = GetArg("-anonymizegkcamount", 0);
//    if (nAnonymizeGkcAmount > 999999) nAnonymizeGkcAmount = 999999;
//    if (nAnonymizeGkcAmount < 2) nAnonymizeGkcAmount = 2;
    ...
    LogPrintf("Anonymize GKC Amount %d\n", nAnonymizeGkcAmount);
    ...
    /* Denominations
       ...
       For example:
       1GKC+1000 == (.1GKC+100)*10
       10GKC+10000 == (1GKC+1000)*10
    */
