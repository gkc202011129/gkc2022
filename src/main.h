...
static const unsigned int MAX_STANDARD_TX_SIZE = 200*1024;
static const unsigned int MAX_STANDARD_VIN_COUNT = 512;
static const unsigned int MAX_STANDARD_VOUT_COUNT = 512;
...
CAmount GetMasternodePayment(BlockHeight historyHeight, int64_t blockValue, int nMasternodeCount = 0);
...
extern std::string appNodeLabelMode;
...
extern int64_t timeOfLastBackupWallet;

namespace advertisement
{
	CAmount GetAdPrice(BlockHeight);
}

template <typename Value, typename Height>
Value halve(Value init, Height start, Height interval, Height h)
{
	Value v = init;
	if(h >= start)
	{
		const Height offset = h - start;
		const int n = offset / interval;
		v = (v >> n);
	}
	return v;
}

extern std::map<CBitcoinAddress,int> banAddressMap; // map address to start height
