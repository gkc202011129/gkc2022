...
#if defined(NDEBUG)
#error "GKC cannot be compiled without assertions."
#endif
...
{
	if(state==nullptr)
		throw std::runtime_error("state==nullptr! in UpdatePreferredDownload()");
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state)
{
    ...
    BOOST_FOREACH (const CTxOut& txout, tx.vout) {
		...
		if (EnableBanAddress() && HasAddressBanned(txout.GetScriptAddresses(), chainActive.Height())) {
			return state.DoS(100, error("CheckTransaction() : address of txout in blacklist"),
								 REJECT_INVALID, "bad-txns-vout-blacklist");
		}
		...
		if(chainActive.Height() >= 69671)
		{
			ScriptEx scriptex(txout.scriptPubKey);
			if(scriptex.IsAdvertise()  && txout.nValue < advertisement::GetAdPrice(chainActive.Height() + 1))
				return state.DoS(100, error("CheckTransaction() : business fee of AD too low"),REJECT_INVALID, "bad-ad-fee");
			
			if((scriptex.IsSysMessage() || scriptex.IsFilterAd()) && !advertisement::IsSignByAdministrator(txout.scriptPubKey))
				return state.DoS(100, error("CheckTransaction() : this system message is not signed by administrator"),REJECT_INVALID, "bad-admin-tx");
		}
    }
    ...
    // Check for duplicate inputs, and blacklist wallet
    ...
    for (const CTxIn& txin : tx.vin) {
		...
		if (EnableBanAddress()) {
			...
			if (coins && txin.prevout.n < coins->vout.size() && 
				HasAddressBanned(coins->vout[txin.prevout.n].GetScriptAddresses(), chainActive.Height())) {
				return state.DoS(100, error("CheckTransaction() : blacklist inputs"),
								 REJECT_INVALID, "bad-txns-inputs-blacklist");
			}
		}
    }
    ...
    if (tx.IsCoinBase()) {
		...
		if(!tx.IsCoinBase2())
		{
			if(!Entrustment::GetInstance().CheckTransaction(tx))
			{
				return state.DoS(100, error("CheckTransaction() : Entrustment::CheckTransaction() failed!"),
								 REJECT_INVALID, "bad-dpos-tx");
			}
		}
#endif
    }
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    uint256 hash = tx.GetHash();
    if (pool.exists(hash)) {
        LogPrintf("%s tx[%s] already in mempool\n", __func__,hash.GetHex().c_str());
        return false;
    }
...
bool GetTransaction(const uint256& hash, CTransaction& txOut, uint256& hashBlock, bool fAllowSlow)
{
    CBlockIndex* pindexSlow = NULL;
    {
		...
		if(hash == Params().GenesisBlock().vtx[0].GetHash())
		{
			...
			txOut = Params().GenesisBlock().vtx[0];
			return true;
		}
...
bool WriteBlockToDisk(CBlock& block, CDiskBlockPos& pos)
{
	...
	//LogPrintf("DPOS WriteBlockToDisk | %s\n",block.ToString());
...
CAmount GetBlockValue(BlockHeight historyHeight)
{
	const BlockHeight newHeight = historyHeight + 1;
	crp::BlockValue bv = crp::GetBlockValue(newHeight);
	return bv.Total();
}

CAmount GetMasternodePayment(BlockHeight historyHeight, int64_t blockValue, int nMasternodeCount)
{
	const BlockHeight newHeight = historyHeight + 1;
	crp::BlockValue bv = crp::GetBlockValue(newHeight);
	return bv.masternode;
}
...
{
	//LogPrintf("DPOS UpdateCoins | nHeight=%d, tx=%s\n",nHeight,tx.ToString());
    // mark inputs spent
    if (!tx.IsCoinBase() && !tx.IsZerocoinSpend()) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin) {
			...
			if(!ret)
				LogPrintf("ERROR: UpdateCoins | assert failed | tx.hash=%s\n",tx.GetHash().ToString());
            assert(ret);
        }
    }
	...
	CCoinsModifier outs = inputs.ModifyCoins(tx.GetHash());
	//LogPrintf("DPOS UpdateCoins | *outs=%s\n",(*outs).ToString());
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from GKC
        ...
        // specially with outsEmpty.
        {
			...
			//LogPrintf("DPOS DisconnectBlock | before ClearUnspendable: *outs=%s\n",(*outs).ToString());
            ...
            if (*outs != outsBlock)
            {
            	//LogPrintf("DPOS DisconnectBlock | *outs=%s, outsBlock=%s, tx=%s\n",(*outs).ToString(),outsBlock.ToString(),tx.ToString());
                fClean = fClean && error("DisconnectBlock() : added transaction mismatch? database corrupted");
            }

#ifdef DPOS
			bool debugHit = false;
			for(const CTxOut& txout : outs->vout)
			{
				if(txout.type != CTxOut::Type::DEFAULT)
				{
					debugHit = true;
					break;
				}
			...
			if(debugHit)
			{
            	//LogPrintf("DPOS DisconnectBlock | debugHit, *outs=%s\n",(*outs).ToString());
			}
#endif
...
#ifdef DPOS
	Entrustment::GetInstance().RevertToHeight(pindex->pprev->nHeight);
#endif
...
{
    RenameThread("gkc-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZGKCMinted()
{
...
void RecalculateZGKCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGKC supply
...
bool RecalculateGKCSupply(int nHeightStart)
{
...
{
    // GKC: recalculate Accumulator Checkpoints that failed to database properly
...
{
	LogPrint("gkc","ConnectBlock | info | pindex->nHeight=%s, block=%s\n",pindex->nHeight, block.GetHash().ToString());
    AssertLockHeld(cs_main);
    ...
    if (crp::IsPowHeight(pindex->nHeight) && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active"),
                         REJECT_INVALID, "PoS-early");

    if (crp::IsPosHeight(pindex->nHeight) && block.IsProofOfWork())
    ...
    {
        LogPrintf("ConnectBlock | ERROR | pindex->nHeight=%d, Params().Contract_StartHeight()=%d, pindex->version=%d\n",pindex->nHeight,Params().Contract_StartHeight(),pindex->nVersion);
    ...
    if (pindex->nHeight == Params().Contract_StartHeight())
    {
        ...
        {
			LogPrintf("ConnectBlock | block.GetVMState() ERROR | pindex->nHeight=%d, pindex->version=%d\n",pindex->nHeight,pindex->nVersion);
    ...
    } else if(pindex->nHeight > Params().Contract_StartHeight())
    {
    ...
    //-----------------gkc-vm-----------------------
    //Note: No need to add gkcDGP here, Will exam in ContractTxConnectBlock
	...
	CAmount totalEntrustReward = 0;
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        const CTransaction& tx = block.vtx[i];

		//LogPrintf("debug | ConnectBlock | dpos | block.vtx[i=%u]=%s\n",i,tx.ToString());
        ...
        bool hasOpSpend = tx.HasOpSpend(); //gkc-vm
        ...
        else
        {
			...
			if(tx.IsCoinStake()){
				totalEntrustReward += tx.GetEntrustRewardValueOut();
			}
        }
	...
	LogPrint("zero","ConnectBlock | pindex->nHeight=%d, BlockToZerocoinMintList() return listMints.size()=%u\n",pindex->nHeight,listMints.size());
   ...
   // LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime1 - nTimeStart), 0.001 * (nTime1 - nTimeStart) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime1 - nTimeStart) / (nInputs - 1), nTimeConnect * 0.000001);
	...
	crp::BlockValue bv = crp::GetBlockValue(pindex->nHeight);
    CAmount nExpectedMint = bv.Total() - bv.entrust + totalEntrustReward; 

	nExpectedMint += nFees;

    if (!IsBlockValueValid(block, nExpectedMint, pindex->nMint)) {
        return state.DoS(100,
                         error("ConnectBlock() : miner reward pays too much (actual=%s vs limit=%s)",
    ...
    if (!fVerifyingBlocks && pindex->nHeight >= Params().Zerocoin_StartHeight() && pindex->nHeight % 10 == 0) {
        ...
        if (!CalculateAccumulatorCheckpoint(pindex->nHeight, nCheckpointCalculated)) {
	        ...
	        return error("ConnectBlock() : failed to calculate accumulator checkpoint because wallet is shutting down");
        }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert GKC to zGKC
...
bool static ConnectTip(CValidationState& state, CBlockIndex* pindexNew, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    LogPrintf("bench - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (!rv) {
            ...
            UpdateState(oldHashStateRoot, oldHashUTXORoot);//gkc-vm
            ClearCacheResult();//gkc-vm
...
static bool ActivateBestChainStep(CValidationState& state, CBlockIndex* pindexMostWork, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    int64_t clock_revert = GetTimeMicros();
    while (chainActive.Tip() && chainActive.Tip() != pindexFork) {
        if (!DisconnectTip(state))
            return false;
    }

    int64_t clock_connect = GetTimeMicros();
	...
	LogPrint("clock","ActivateBestChainStep | info,clock | findfork=%lld, revert=%lld, connect=%lld, total=%lld\n", clock_revert-clock_begin, clock_connect-clock_revert, clock_end-clock_connect, clock_end-clock_begin);
    return true;
}
...
{
	LogPrint("gkc","ActivateBestChain | info | fAlreadyChecked=%d\n",fAlreadyChecked);
    ...
    do {
        ...
        while (true) {
            ...
            if (pindexMostWork == NULL || pindexMostWork == chainActive.Tip()){
				LogPrint("gkc","ActivateBestChain | info | pindexMostWork=%p\n",pindexMostWork);
                return true;
            }
...
//			LogPrint("dpos","CheckBlockHeader | 1 | block.GetBlockTime()=%lld, Params().Zerocoin_StartTime()=%lld, block.nVersion=%d, Params().Zerocoin_HeaderVersion()=%d\n",block.GetBlockTime(),Params().Zerocoin_StartTime(),block.nVersion,Params().Zerocoin_HeaderVersion());
//            return state.DoS(50, error("CheckBlockHeader() : block version must be above 4 after ZerocoinStartHeight"),
//                             REJECT_INVALID, "block-version");
//        }
//    } else {
//        if (block.nVersion >= Params().Zerocoin_HeaderVersion()){
//			LogPrint("dpos","CheckBlockHeader | 2 | block.GetBlockTime()=%lld, Params().Zerocoin_StartTime()=%lld, block.nVersion=%d, Params().Zerocoin_HeaderVersion()=%d\n",block.GetBlockTime(),Params().Zerocoin_StartTime(),block.nVersion,Params().Zerocoin_HeaderVersion());
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if(block.IsContractEnabled()){
            ...
            //Don't allow contract opcodes in coinbase   //gkc-vm
    ...
    bool lastWasContract = false; //gkc-vm
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfWork() && crp::IsPowHeight(pindexPrev->nHeight)) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
	...
	const bool debug = nHeight==33972 || hash==uint256("e3e64e6799c7060672fdbf69e2850214e6dc251922913cb9c5d38a18d1edaa3d");
	if(debug)
		LogPrintf("ContextualCheckBlockHeader | block#33972=%s\n",hash.ToString());
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nVersion >= 2 &&
        ...
        if (block.vtx[0].vin[0].scriptSig.size() < expect.size() ||
            ...
            LogPrintf("warnning | ContextualCheckBlock | nHeight=%d, expect=%s, block.vtx[0].vin[0].scriptSig=%s\n",nHeight,expect.ToString(),block.vtx[0].vin[0].scriptSig.ToString());
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
	...
	const bool debug = hash==uint256("e3e64e6799c7060672fdbf69e2850214e6dc251922913cb9c5d38a18d1edaa3d");
	if(debug)
		LogPrintf("AcceptBlockHeader | block#33972=%s\n",hash.ToString());
...
{
	LogPrint("gkc","AcceptBlock | info | block=%s\n",block.GetHash().ToString());
    AssertLockHeld(cs_main);

	const bool debug = block.GetHash()==uint256("e3e64e6799c7060672fdbf69e2850214e6dc251922913cb9c5d38a18d1edaa3d");
	if(debug)
		LogPrintf("AcceptBlock | block#33972=%s\n",block.GetHash().ToString());
    ...
    if (block.GetHash() != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            ...
            if (Checkpoints::CheckBlock(pindexPrev->nHeight, block.hashPrevBlock, true)) {
                ...
                ReconsiderBlock(statePrev, pindexPrev);
                if (statePrev.IsValid()) {
					//LogPrintf("DPOS AcceptBlock | call ActivateBestChain\n");
                    ActivateBestChain(statePrev);
                    return true;
                }
            }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	const bool debug = pblock->GetHash()==uint256("e3e64e6799c7060672fdbf69e2850214e6dc251922913cb9c5d38a18d1edaa3d");
	if(debug)
		LogPrintf("ProcessNewBlock | block#33972=%s\n",pblock->GetHash().ToString());
...

    {
        ...
        CheckBlockIndex ();
		int64_t clock_end = GetTimeMicros();
		LogPrint("clock","ProcessNewBlock | info,clock | AcceptBlock=%lld\n",clock_end-clock_begin);
        if (!ret)
...
bool static LoadBlockIndexDB(string& strError)
{
        ...
        if (vSortedByHeight.size() > nHeightLastBlockFile && pcoinsTip->GetBestBlock() != vSortedByHeight[nHeightLastBlockFile].second->GetBlockHash()) {
            ...
            while (nSortedPos < vSortedByHeight.size()) {
                ...
                for (unsigned int i = 0; i < block.vtx.size(); i++) {
					...
					LogPrint("wallet","LoadBlockIndexDB | block=%s, i=%d, tx=%s\n",block.GetHash().ToString(),i,block.vtx[i].ToString());
    ...
    pblocktree->ReadFlag("logevents", fLogEvents);//gkc-vm
    LogPrintf("logevents %s", fLogEvents ? "enabled" : "disabled");//gkc-vm
...
#ifdef DPOS
    LogPrint("dpos","LoadBlockIndexDB | info | load block, and count entrustment, chainActive.Height()=%d\n", chainActive.Height());
	CBlock block;
	const BlockHeight topheight = chainActive.Height();
	const BlockHeight offsetHeight = 1000;
	const BlockHeight lastBackupHeight = topheight - topheight%offsetHeight;
	const BlockHeight minHeightNum = GetArg("-checkblocks", 100);
	BlockHeight backupHeight = (topheight - lastBackupHeight + 1 >= minHeightNum ? lastBackupHeight : lastBackupHeight - offsetHeight);
	BlockHeight startHeight = 0;
	while(backupHeight > 0){
		if(Entrustment::GetInstance().ReadFromBackupFile(backupHeight)){
			startHeight = backupHeight + 1;
			break;
		}
		backupHeight -= offsetHeight;
	}
    LogPrint("dpos","LoadBlockIndexDB | info | load block, and count entrustment, startHeight=%d, topheight=%d\n", startHeight, topheight);
	int64_t clock_begin = GetTimeMillis();
	for(BlockHeight h=startHeight; h<=topheight; h++){
		CBlockIndex* pindex = chainActive[h];
		assert(pindex);
		dpos::AddMetaBlock(pindex);
		if (ShutdownRequested()){
			LogPrint("dpos","LoadBlockIndexDB | shutdown break! h=%d\n",h);
			break;
		}
	}
	int64_t clock_stop = GetTimeMillis();
	LogPrint("dpos","LoadBlockIndexDB | read %d height in %lld ms, avg= %d ms/h \n",topheight-startHeight+1,clock_stop-clock_begin,(clock_stop-clock_begin)/(topheight-startHeight+1));
#endif
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    uint256 oldHashStateRoot, oldHashUTXORoot;
    GetState(oldHashStateRoot, oldHashUTXORoot);//gkc-vm
...
{
	dpos::DposMetaBlockIO::GetInstance().LoadLastBlockPos();
...
bool InitBlockIndex()
{
	LogPrint("gkc","InitBlockIndex | info | fReindex=%d\n",fReindex);
    LOCK(cs_main);
    // Check whether we're already initialized
    if (chainActive.Genesis() != NULL) {
		LogPrint("gkc","InitBlockIndex | info | chainActive.Genesis() != NULL, we're already initialized\n");
        return true;
    }
...
bool IsAppNode(const std::string& label)
{
	return label.find(appNodeLabelMode) != label.npos;
}

bool IsBanAppNodeWitchOn()
{
	return !appNodeLabelMode.empty();
}

void BanAppNode(const std::string& labelMode)
{
	LogPrint("control","BanAppNode | new label: %s, previous: %s\n",labelMode,appNodeLabelMode);
	appNodeLabelMode = labelMode;
}

void CancelBanAppNode()
{
	LogPrint("control","CancelBanAppNode | previous label: %s\n",appNodeLabelMode);
	appNodeLabelMode.clear();
}

int64_t timeOfLastBackupWallet = 0;
void BackupWallet()
{
	...
	int nWalletBackups = GetArg("-createwalletbackups", 100000000);
	if (nWalletBackups > 0) {
		if (filesystem::exists(backupDir)) {
			...
			if (!BackupWallet(*pwalletMain, backupFile.string()))
				LogPrintf("BackupWallet | Error: Wallet backup failed!\n");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    RandAddSeedPerfmon();
    if (fDebug)
        LogPrintf("received: %s (%u bytes) peer=%d, pfrom->nVersion=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->id, pfrom->nVersion);

	if(GetTime() >= timeOfLastBackupWallet + 60*GetArg("-backupwalletinterval", 1*24*60))
	{
		timeOfLastBackupWallet = GetTime();
		BackupWallet();
	}
    ...
    if (strCommand == "version") {
        ...
        // GKC: We use certain sporks during IBD, so check to see if they are
		...
		const bool fBanAppNode = !appNodeLabelMode.empty();
		if(fBanAppNode && IsAppNode(pfrom->cleanSubVer)){
            pfrom->PushMessage("reject", strCommand, REJECT_INVALID, string("Not accept app node"));
            LOCK(cs_main);
        ...
        {
        	LOCK(cs_main);
			CNodeState* fromNodeState = State(pfrom->GetId());
			if(fromNodeState==nullptr)
				return false;
        	UpdatePreferredDownload(pfrom,fromNodeState);
       	}
		...
		//LogPrintf("DPOS ProcessMessage | get tx=%s,vRecv=%s,tx.tostring=%s\n",tx.GetHash().GetHex().c_str(),vRecv.str().c_str(),tx.ToString().c_str());
	...
	#ifdef DPOS
    else if ((strCommand == "getagent")) {
		int blockHeight = -1;

		//todo parse argument: blockHeight

		if(blockHeight == -1)
			blockHeight = chainActive.Height();
		
		pfrom->ClearAgentList();
		pfrom->SetAgentBlockHeight(blockHeight);
		std::vector<Agent> agents = Entrustment::GetInstance().GetAgentList(blockHeight);
		LogPrint("dpos","ProcessMessage | agents.size=%d\n",agents.size());
		for (const Agent& agent : agents) {
			pfrom->PushAgent(agent);
		}
    }
	#endif
...
std::map<CBitcoinAddress,int> banAddressMap;

bool EnableBanAddress()
{
	return banAddressMap.size() > 0;
}
...
static std::vector<string> split(string str, char delim)
{
    ...
    std::string token;
    while (std::getline(strs, token, delim)) {
        result.push_back(token);
    }
	return result;
}

void InitBanAddressMap(std::vector<string> banAddressVec)
{
	for (size_t i=0; i<banAddressVec.size(); i++) {
		std::vector<string> params = split(banAddressVec[i],':');
		string addr;
		int height = 0;
		if (params.size() >= 1) {
			addr = params[0];
		...
		AddBanAddress(CBitcoinAddress(addr), height);
	}
}

bool IsAddressBanned(CBitcoinAddress addr, int chainHeight)
{
	if (banAddressMap.count(addr) > 0) {
		return chainHeight >= banAddressMap[addr];
	}
	return false;
}

bool HasAddressBanned(std::vector<CBitcoinAddress> addrs, int chainHeight)
{
	for (size_t i=0; i<addrs.size(); i++) {
		if (IsAddressBanned(addrs[i],chainHeight)) {
			return true;
		}
	}
	return false;
...
void AddBanAddress(CBitcoinAddress address, int startHeight)
{
	banAddressMap[address] = startHeight;
}
