...
public:
    ...
    GkcDGP(GkcState *_state, bool _dgpevm = true) : dgpevm(_dgpevm), state(_state)
    {
        initDataEIP158();
    }
...
private:
    ...
    const GkcState *state;

    dev::Address templateContract;

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageDGP; //gkc hashedKey,<key,value>

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageTemplate;//gkc hashedKey,<key,value>

    std::vector<unsigned char> dataTemplate;   //gkc call contract ,res->output
