...
using plusAndMinus = std::pair<dev::u256, dev::u256>;  //gkc commmetn the first: the plus value;the second:the minus value
...
class GkcState : public dev::eth::State
{
...
public:
    ...
    GkcState(dev::u256 const &_accountStartNonce, dev::OverlayDB const &_db, const std::string &_path,
              dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute
    execute(dev::eth::EnvInfo const &_envInfo, dev::eth::SealEngineFace const &_sealEngine, GkcTransaction const &_t,
...
private:
    ...
    dev::Address createGkcAddress(dev::h256 hashTx, uint32_t voutNumber);
...
{
    std::unique_ptr<GkcState> &globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<GkcState> &_globalStateRef) :
...
public:
    ...
    CondensingTX(GkcState *_state, const std::vector<TransferInfo> &_transfers, const GkcTransaction &_transaction,
...
private:
    ...
    const GkcTransaction &transaction;
