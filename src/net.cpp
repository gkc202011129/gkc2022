...
void CNode::CloseSocketDisconnect()
{
    fDisconnect = true;
   	...
   	{
		TRY_LOCK(cs_hSocket, lockSocket);
...
void CNode::Ban(const CSubNet& subNet, const BanReason &banReason, int64_t bantimeoffset, bool sinceUnixEpoch)
{
    ...
    if (bantimeoffset <= 0)
    {
        ...
        bantimeoffset = GetArg("-bantime", 3); // todo: recover to 24 hour
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (GKC users should be well used to small numbers with many decimal places by now :)
...
void SocketSendData(CNode* pnode)
{
    ...
    while (it != pnode->vSendMsg.end()) {
		...
		int nBytes = 0;
       	{
			TRY_LOCK(pnode->cs_hSocket, lockSocket);
    ...
    while (true) {
        ...
        if (nSelect == SOCKET_ERROR) {
            if (have_fds) {
				...
				{
					if(i >= FD_SETSIZE){
						LogPrintf("error | ThreadSocketHandler | line=%d | i=%u,hSocketMax=%u\n",__LINE__,i,hSocketMax);
					}
                    FD_SET(i, &fdsetRecv);
                }
            }
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "GKC " + FormatFullVersion();
...
void static ThreadStakeMinter()
{
    ...
    try {
        ...
        LogPrintf("ThreadStakeMinter() std::exception: %s \n",e.what());
    } catch (...) {
        LogPrintf("ThreadStakeMinter() error,  other type of exception\n");
    }
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. GKC Core is probably already running."), addrBind.ToString());
        else
...
{
	//LogPrintf("DPOS RelayTransaction | tx=%s, ss=%s, tx.tostring=%s\n",tx.GetHash().GetHex().c_str(),ss.str().c_str(),tx.ToString().c_str());
...
#ifdef DPOS
void CNode::ClearAgentList()
{
	agentList.clear();
}
void CNode::SetAgentBlockHeight(int height)
{
	agentBlockHeight = height;
}
void CNode::PushAgent(const Agent& a)
{
	agentList.push_back(a);
}
void CNode::PushAentListMessage()
{
	if(!agentList.empty())
	{
		LogPrint("dpos","CNode::PushAentListMessage | agentList.size=%d\n",agentList.size());
		{//v1
			CDataStream ss(SER_NETWORK, 1);
			ss.reserve(1000);
			ss << agentBlockHeight << agentList;
			PushMessage("agent", ss);
		}
		{//v2
			uint16_t version = 3;
			CDataStream ss(SER_NETWORK, version);
			ss.reserve(1000);
			ss << (uint16_t)version << agentBlockHeight << agentList;
			PushMessage("agent_v2", ss);
		}
		agentList.clear();
	}
}
